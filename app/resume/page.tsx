...
export default function ResumePage() {
  ...
  const [selectedPreviewResume, setSelectedPreviewResume] = useState<number | null>(null)
      ...
      <div className="max-w-full p-4 sm:p-6 lg:p-8">
        <div className="space-y-6">
                    ...
                    <div className="flex justify-between items-center pt-2">
                      <div className="flex space-x-2">
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => setSelectedPreviewResume(resume.id)}
                        >
          ...
          {selectedPreviewResume && (
            <Dialog open={!!selectedPreviewResume} onOpenChange={(open) => !open && setSelectedPreviewResume(null)}>
              <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                ...
                {(() => {
                  const selectedResume = resumes.find((r) => r.id === selectedPreviewResume)
                  if (!selectedResume) return null
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-6 border border-blue-200">
                        <div className="flex justify-between items-start mb-4">
                          <div>
                            <h1 className="text-3xl font-bold text-gray-900">{selectedResume.title}</h1>
                            <p className="text-lg text-gray-600 mt-1">
                              {selectedResume.targetRole}
                              {selectedResume.company && ` • ${selectedResume.company}`}
                            </p>
                          </div>
                          <div className="text-right">
                            <Badge className={getStatusColor(selectedResume.status)}>
                              {getStatusIcon(selectedResume.status)}
                              <span className="ml-1 capitalize">{selectedResume.status}</span>
                            </Badge>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                          <div>
                            ...
                            <p className="font-semibold text-gray-900 capitalize">{selectedResume.type}</p>
                          </div>
                          <div>
                            <p className="text-sm text-gray-600">Version</p>
                            <p className="font-semibold text-gray-900">{selectedResume.version}</p>
                          </div>
                          <div>
                          ...
                          <div>
                            ...
                            <p className="font-semibold text-gray-900">{selectedResume.fileSize}</p>
                          </div>
                        </div>
                      </div>
                      ...
                      {selectedResume.fileName && (
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                          <div className="flex items-center gap-2">
                            <FileText className="w-5 h-5 text-blue-600" />
                            <div className="flex-1">
                              <p className="text-sm font-medium text-blue-700">Uploaded File</p>
                              <p className="text-blue-600">{selectedResume.fileName}</p>
                            </div>
                          </div>
                        </div>
                      )}
                        ...
                        <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg p-4 border border-purple-200">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              <Zap className="w-5 h-5 text-purple-600" />
                              <div>
                                <p className="text-sm font-medium text-purple-700">ATS Score</p>
                                <p className="text-xs text-purple-600">Applicant Tracking System Score</p>
                              </div>
                            </div>
                            <div className="text-right">
                              <p className="text-3xl font-bold text-purple-600">{selectedResume.atsScore}</p>
                              <p className="text-xs text-purple-600">/100</p>
                            </div>
                          </div>
                        </div>
                        ...
                        {selectedResume.notes ? (
                          <div className="space-y-4">
                            <div className="bg-gray-50 p-4 rounded-lg">
                              <p className="text-gray-700 whitespace-pre-wrap leading-relaxed">{selectedResume.notes}</p>
                            </div>
                          </div>
                        ) : (
                          <p className="text-gray-500 italic">No notes or content available for this resume.</p>
                        )}
                      </div>
                      ...
                      {selectedResume.feedback.length > 0 && (
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                          <h3 className="font-semibold text-blue-900 mb-3">Feedback</h3>
                          <ul className="space-y-2">
                            {selectedResume.feedback.map((feedback, idx) => (
                      ...
                      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="font-semibold text-yellow-900 mb-2">Rating</p>
                            <div className="flex items-center gap-1">
                              {[...Array(5)].map((_, i) => (
                                <Star
                                  key={i}
                                  className={`w-5 h-5 ${
                                    i < selectedResume.rating
                                      ? "text-yellow-400 fill-current"
                                      : "text-gray-300"
                                  }`}
                                />
                              ))}
                              <span className="text-sm text-yellow-700 ml-2">({selectedResume.rating}/5)</span>
                            </div>
                          </div>
                          <div className="text-right">
                            <p className="text-sm text-gray-600">Downloads</p>
                            <p className="text-2xl font-bold text-gray-900">{selectedResume.downloadCount}</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="bg-gray-50 p-4 rounded-lg">
                          ...
                          <p className="font-semibold text-gray-900">{selectedResume.targetRole}</p>
                        </div>
                        {selectedResume.company && (
                          <div className="bg-gray-50 p-4 rounded-lg">
                            <p className="text-sm text-gray-600 mb-1">Company</p>
                            <p className="font-semibold text-gray-900">{selectedResume.company}</p>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button
                          onClick={() => setSelectedPreviewResume(null)}
                          className="flex-1"
                        >
                          Close Preview
                        </Button>
